...
class norme:
    ...
    def check_nbchar(self):
        ...
        if (line[81:]):
            self.print_error('ligne de plus de 80 colonnes')
    ...
    def check_cfunc(self):
        p = re.compile('[ \t](if|else|return|while|for|__attribute__)(\()')
        test = re.search(p, self.line)
        if test:
    ...
    def check_sys_include(self):
        if self.line[:1] == "#" and self.line[-2:] == "\"\n":
            ...
            if self.line[:1] == "#" and self.line[-2:] == ">\n" and self.sys_include == 1:
                self.print_error('Header systeme mal place')
    ...
    def check_malloc(self):
        ...
        if test and (self.file != "xmalloc.c"):
            self.print_error('Malloc non verifie')
        ...
        if self.file[-2:] == ".h":
            if self.line[:1] != '\n':
                if self.double_inclusion != 1:
                    if self.line[-4:] != "_H_\n":
                        self.print_error('Header non protege')
                    else:
                ...
                if file[-2:] == '.c' or file[-2:] == '.h':
                    ...
                    try:
                        ...
                        for self.line in fd.readlines():
                            if self.nb_line <= 9 and self.header == 1:
                                self.check_header()
                            else:
                                self.check_line()
...
def help():
    ...
    print "-noheader: considerer que les fichiers n'ont pas de header"
